...
class QuestionFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?
        ...
        title.text = getString(q_title[questionType])

        val qTextView = listOf<TextView>(
        ...
        //텍스트 가져오기
        for (i in qTextView.indices){
            qTextView[i].text = getString(q_texts[questionType][i])
            ...
            radioButton1.text = getString(q_answers[questionType][i][0])
            radioButton2.text = getString(q_answers[questionType][i][1])
        }
        return view
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btn_next.setOnClickListener {
            ...
            if (isAllAnswered){
                ...
                (activity as? TestActivity)?.questionnaireResults?.aadResponse(responses)
                (activity as? TestActivity)?.moveToNext()
